...
abstract class StorageDao {
    ...
    abstract suspend fun isPrefixInCache(
        chainId: String,
        keyPrefix: String,
    ): Boolean

    @Query("SELECT EXISTS($SELECT_FULL_KEY_QUERY)")
    abstract suspend fun isFullKeyInCache(
        chainId: String,
        fullKey: String,
    ): Boolean
    ...
    abstract suspend fun removeByPrefix(
        prefix: String,
        chainId: String,
    )
    ...
    abstract suspend fun removeByPrefixExcept(
        prefix: String,
        exceptionFullKeys: List<String>,
        chainId: String,
    )

    @Query(SELECT_FULL_KEY_QUERY)
    abstract fun observeEntry(
        chainId: String,
        fullKey: String,
    ): Flow<StorageEntryLocal?>

    @Query(SELECT_PREFIX_KEY_QUERY)
    abstract fun observeEntries(
        chainId: String,
        keyPrefix: String,
    ): Flow<List<StorageEntryLocal>>

    @Query("SELECT storageKey from storage WHERE chainId = :chainId  AND storageKey LIKE :keyPrefix || '%'")
    abstract suspend fun getKeys(
        chainId: String,
        keyPrefix: String,
    ): List<String>
    ...
    abstract suspend fun filterKeysInCache(
        chainId: String,
        keys: List<String>,
    ): List<String>

    @Transaction
    open suspend fun insertPrefixedEntries(
        entries: List<StorageEntryLocal>,
        prefix: String,
        chainId: String,
    ) {
