...
| `merchant_url` | string | Merchant mode (optional) | `feature/products` | DotNS name of the merchant terminal opened from Settings, e.g. `terminal.dot`. A full URL is accepted and reduced to its host, so any path is ignored. Unset falls back to `terminal` on the active network's TLD. |
