package io.paritytech.polkadotapp.common.data.substrate

import io.novasama.substrate_sdk_android.runtime.RuntimeSnapshot
import io.novasama.substrate_sdk_android.runtime.definitions.types.*
...
fun incompatible(cause: Throwable? = null): Nothing = throw IllegalStateException("Binding is incompatible", cause)
...
fun RuntimeMetadata.storageReturnType(
    moduleName: String,
    storageName: String,
): Type<*> {
...
fun <D> RuntimeType<*, D>.fromHexOrIncompatible(
    scale: String,
    runtime: RuntimeSnapshot,
): D =
    successOrIncompatible {
        fromHex(runtime, scale)
    }

fun <D> RuntimeType<*, D>.fromByteArrayOrIncompatible(
    scale: ByteArray,
    runtime: RuntimeSnapshot,
): D =
    successOrIncompatible {
...
private fun <T> successOrIncompatible(block: () -> T): T =
    runCatching {
        block()
    }.getOrElse { incompatible(it) }
