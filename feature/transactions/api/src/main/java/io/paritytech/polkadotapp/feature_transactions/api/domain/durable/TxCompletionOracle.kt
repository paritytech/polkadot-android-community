...
interface LedgerView {
    val transactions: List<DurableTxFacts>
...
interface TxCompletionOracle {
    ...
    interface PassScope {
         ...
         */
        fun provenCompleted(tx: DurableTxFacts, head: HeadKind): Boolean
         ...
         */
        fun provenNotCompleted(tx: DurableTxFacts, head: HeadKind): Boolean = false
    }
    ...
    companion object {
         ...
         */
        val Unobservable = object : TxCompletionOracle {
            override suspend fun openPass(
                transactions: List<DurableTxFacts>,
                ledger: LedgerView,
                view: PinnedChainView,
            ): Result<PassScope> = Result.success(
                object : PassScope {
                    override fun provenCompleted(tx: DurableTxFacts, head: HeadKind) = false
                }
            )
        }
    }
