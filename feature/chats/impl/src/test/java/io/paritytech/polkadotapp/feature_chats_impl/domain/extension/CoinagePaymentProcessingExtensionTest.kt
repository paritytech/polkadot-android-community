...
class CoinagePaymentProcessingExtensionTest {
    ...
    private fun stateOf(status: CoinagePaymentStatus) = CoinagePaymentState(
        coin = Coin(derivationIndex = 0, valueExponent = ValueExponent(3), age = Coin.Age.Unknown, isOnChain = false, accountId = accountId(0), provenance = CoinProvenance.UNKNOWN),
        status = status,
    )
