...
class UsernamesClaimInteractorTest {
    ...
    @Test
    fun `claim failure reports Failed with the original error`() = runTest {
        val error = RuntimeException("network down")
        whenever(usernameRepository.claimUsername(eq(claimParams))).thenReturn(Result.failure(error))

        val outcome = interactor.claimUsername(username, preferredDigits = "07")

        assertEquals(ClaimUsernameOutcome.Failed(error), outcome)
    }
