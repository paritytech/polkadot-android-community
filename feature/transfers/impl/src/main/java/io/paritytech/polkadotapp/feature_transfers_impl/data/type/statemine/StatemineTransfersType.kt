package io.paritytech.polkadotapp.feature_transfers_impl.data.type.statemine
...
import io.paritytech.polkadotapp.chains.multiNetwork.chain.model.hasSameId
...
class StatemineTransfersType @AssistedInject constructor(
    extrinsicService: ExtrinsicService,
    private val chainRegistry: ChainRegistry,
    @Assisted asset: Chain.Asset,
) : SubstrateTokenTransfersType(extrinsicService, chainRegistry, asset) {
    private val statemineType = asset.requireAssets()

    @AssistedFactory
    @Singleton
    interface Factory {
        fun create(asset: Chain.Asset): StatemineTransfersType
    }
    ...
    override fun ExtrinsicBuilder.transfer(
        ...
        call(
            moduleName = statemineType.palletNameOrDefault(),
            callName = "transfer",
            ...
            arguments = mapOf(
                "id" to statemineType.prepareIdForEncoding(runtime).value,
                "target" to AddressInstanceConstructor.constructInstance(runtime.typeRegistry, recipient.value),
    ...
    override suspend fun parseTransferCall(call: GenericCall.Instance): ParsedTransferCall? {
        if (!call.instanceOf(statemineType.palletNameOrDefault(), "transfer")) return null

        return runCatching {
            val runtime = chainRegistry.getRuntime(chainAsset.chainId)

            val currencyIdRaw = call.arguments["id"].asRawScaleValue()
            val hasSameId = statemineType.hasSameId(runtime, currencyIdRaw)
            if (!hasSameId) return null

            ParsedTransferCall(
                recipient = bindAccountId(call.arguments["target"]),
