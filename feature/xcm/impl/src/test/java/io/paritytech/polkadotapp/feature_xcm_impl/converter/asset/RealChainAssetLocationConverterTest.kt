...
import io.paritytech.polkadotapp.feature_xcm_api.multiLocation.MultiLocation.Junction.GeneralIndex
import io.paritytech.polkadotapp.feature_xcm_api.multiLocation.MultiLocation.Junction.GlobalConsensus
import io.paritytech.polkadotapp.feature_xcm_api.multiLocation.MultiLocation.Junction.PalletInstance
...
class RealChainAssetLocationConverterTest {
    ...
    private val polkadotChainId = Chain.Ids.POLKADOT
    private val polkadotGenesis = Chain.Geneses.POLKADOT

    private val pahChainId = Chain.Ids.PREVIEWNET_ASSET_HUB
    private val pahGenesis = Chain.Geneses.POLKADOT_ASSET_HUB

    private val kusamaChainId = Chain.Ids.KUSAMA
    ...
    private val polkadotLocation = AbsoluteMultiLocation(GlobalConsensus(polkadotGenesis))
    private val pahLocation = AbsoluteMultiLocation(GlobalConsensus(polkadotGenesis), ParachainId(1000))
    private val kusamaLocation = AbsoluteMultiLocation(GlobalConsensus(kusamaGenesis))

    // Token Locations

    private val dotLocation = AbsoluteMultiLocation(GlobalConsensus(polkadotGenesis))

    private val usdcLocation = AbsoluteMultiLocation(
        GlobalConsensus(polkadotGenesis),
        ParachainId(1000),
    ...
    private fun setupAsset(asset: Chain.Asset, chainId: String, assetId: Int, symbol: String) {
        ...
        whenever(asset.symbol).thenReturn(symbol)
    }
