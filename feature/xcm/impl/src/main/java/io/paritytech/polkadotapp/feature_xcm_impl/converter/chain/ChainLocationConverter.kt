...
import io.paritytech.polkadotapp.chains.util.evmChainIdOrNull
import io.paritytech.polkadotapp.common.utils.reversedManyToOne
...
class RealChainLocationConverter(
    ...
    private suspend fun getNetwork(networkId: MultiLocation.NetworkId): Chain {
        return when (networkId) {
            is MultiLocation.NetworkId.Ethereum -> {
                val chainId = Chain.Ids.createEvmChainId(networkId.chainId)
                chainRegistry.getChain(chainId)
            }
            is MultiLocation.NetworkId.Substrate -> {
                chainRegistry.getChain(networkId.genesisHash)
            }
        }
    }
    ...
    private fun Chain.getNetworkId(): MultiLocation.NetworkId {
        ...
        if (evmChainId != null) {
            return MultiLocation.NetworkId.Ethereum(evmChainId)
        }

        return MultiLocation.NetworkId.Substrate(genesisHash)
    }
}
