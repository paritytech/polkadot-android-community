...
import io.paritytech.polkadotapp.feature_xcm_api.message.XcmInstruction
import io.paritytech.polkadotapp.feature_xcm_api.message.XcmMessage
import io.paritytech.polkadotapp.feature_xcm_api.message.asVersionedXcmMessage
import io.paritytech.polkadotapp.feature_xcm_api.message.asXcmMessage
...
import io.paritytech.polkadotapp.feature_xcm_api.multiLocation.multiAssetIdOn
...
internal class RealXcmBuilder(
    ...
    override fun depositAsset(assets: MultiAssetFilter, beneficiary: AccountId) {
    ...
    private fun PendingInstruction.PayFees.constructEstimationInstruction(feeAssetId: MultiAssetId): XcmInstruction.PayFees {
        val fees = when (val mode = mode) {
            is PayFeesMode.Exact -> mode.fee
            is PayFeesMode.Measured -> feeAssetId.withAmount(Balance.ONE) // Use fake amount in pay fees instruction for fee estimation
        }
