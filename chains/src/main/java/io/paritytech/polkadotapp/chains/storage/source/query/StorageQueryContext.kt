...
interface StorageQueryContext : WithRuntime {
    val chainId: ChainId
    ...
    suspend fun <K : Any> StorageEntry.findExistingKeys(
        keysArguments: List<List<Any?>>,
        keyExtractor: (StorageKeyComponents) -> K
    ): Set<K>

    suspend fun StorageEntry.entriesRaw(vararg prefixArgs: Any?): Map<String, String?>

    suspend fun StorageEntry.entriesRaw(keysArguments: List<List<Any?>>): Map<String, String?>
...
suspend fun StorageQueryContext.multi(builderBlock: MultiQueryBuilder.() -> Unit): MultiQueryBuilder.Result {
    contract {
