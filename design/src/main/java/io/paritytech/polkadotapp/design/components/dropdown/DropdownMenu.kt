...
@Composable
fun NovaDropdownMenu(
    modifier: Modifier = Modifier,
    expanded: Boolean,
    onDismissRequest: () -> Unit,
    offset: DpOffset = DpOffset(0.dp, 0.dp),
    properties: PopupProperties = PopupProperties(focusable = true),
    shape: Shape = PolkadotTheme.shapes.medium,
    ...
    if (expandedStates.currentState || expandedStates.targetState) {
        ...
        val popupPositionProvider = DropdownMenuPositionProvider(
            ...
            transformOriginState = calculateTransformOrigin(parentBounds, menuBounds)
        }
...
private val TransitionAnimation = tween<Float>(durationMillis = TransitionDuration)
...
private fun DropdownMenuContent(
    ...
    val transition = rememberTransition(expandedStates, "dropdown_menu_transition")

    val transitionValue by transition.animateFloat(
        transitionSpec = { TransitionAnimation },
        label = "dropdown_transition_value"
    ) {
        if (it) 1f else 0f
    }
    ...
    PolkadotSurface(
        ...
        color = color,
        border = border,
        shadowElevation = if (color.alpha < 1) 0.dp else 8.dp
    ) {
        Box(
